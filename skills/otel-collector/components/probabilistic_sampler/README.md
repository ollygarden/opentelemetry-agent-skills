...
This is the opposite of **tail sampling** (`tail_sampling`), which buffers the **whole trace** and makes one decision after a wait window using trace-wide signals (errors, latency, span count). Head sampling is cheap and immediate but cannot react to span content; tail sampling is content-aware but stateful, requires buffering, adds latency, and needs all spans of a trace to land on the same instance. In **all three modes** the sampler records the effective threshold on the outgoing item — the trace's `tracestate` (`ot=th:...`) for traces, and log attributes (`sampling.threshold` / `sampling.randomness`) for logs. The W3C-randomness modes (`proportional` / `equalizing`) additionally *read* the incoming threshold and compose with it, so chained samplers stay consistent; the default `hash_seed` mode decides locally with its own FNV hash and does not compose (it ignores — and, with `fail_closed`, drops — items that already carry W3C randomness/threshold).
