...
This is the opposite of **tail sampling** (`tail_sampling`), which buffers the **whole trace** and makes one decision after a wait window using trace-wide signals (errors, latency, span count). Head sampling is cheap and immediate but cannot react to span content; tail sampling is content-aware but stateful, requires buffering, adds latency, and needs all spans of a trace to land on the same instance. The threshold is propagated downstream — in the W3C `tracestate` (`ot=th:...`) for traces, and in log attributes (`sampling.threshold` / `sampling.randomness`) for logs — so a later sampler can stay consistent.
